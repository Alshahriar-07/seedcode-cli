"""Generate the embedded default OpenRouter key module (v7.1.0 release builds).
...
def generate(env_file: Path) -> int:
    ...
    module = f'''"""Embedded default OpenRouter credential (v7.1.0 build-time artifact).
...
DEFAULT_API_NOTE = "embedded Seed Code default API (v7.1.0 release build)"
'''
