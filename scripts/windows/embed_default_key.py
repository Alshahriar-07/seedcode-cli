"""Generate the embedded default OpenRouter key module (v6.2.0 release builds).
...
def generate(env_file: Path) -> int:
    ...
    module = f'''"""Embedded default OpenRouter credential (v6.2.0 build-time artifact).
...
DEFAULT_API_NOTE = "embedded Seed Code default API (v6.2.0 release build)"
'''
